                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($paymentMethods as $pm)
                        <div class="bg-white rounded-[2rem] border border-[#0d1b4b]/10 shadow-sm hover:shadow-xl transition-shadow p-8 flex flex-col items-center text-center">
                            @if($pm->logo_path)
                                <img src="{{ Storage::disk('media')->temporaryUrl($pm->logo_path, now()->addHours(2)) }}" class="h-16 object-contain mb-6" alt="{{ $pm->name }}">
                            @else
                            ...
                            @if($pm->qr_path)
                                <div class="mt-auto">
                                    ...
                                    <img src="{{ Storage::disk('media')->temporaryUrl($pm->qr_path, now()->addHours(2)) }}" class="w-32 h-32 object-contain mx-auto border p-1 rounded-xl shadow-inner" alt="QR Code">
                                </div>
                            @endif
                        </div>
                    @endforeach
